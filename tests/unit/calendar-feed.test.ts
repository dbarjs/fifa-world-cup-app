...
describe('buildCalendarFeed', () => {
  ...
  it('renders a Placeholder Pairing in bracket notation', () => {
    ...
    expect(ics).toContain('SUMMARY:W83 vs W84 — Round of 16')
  })
...
describe('revision-driven updates', () => {
  ...
  // Match 93 (round of 16) is still a Placeholder Pairing after the sync, so it
  // models the resolution transition independently of the current data.
  const placeholder = sample.find(m => m.matchNumber === 93)!
  ...
  it('keeps the UID stable when a Placeholder Pairing resolves', () => {
    ...
    expect(before).toContain('UID:wc2026-m93@fifa-world-cup-app')
    expect(after).toContain('UID:wc2026-m93@fifa-world-cup-app')
  })

  it('replaces the placeholder title with the resolved teams', () => {
    expect(before).toContain('SUMMARY:W83 vs W84 — Round of 16')
    expect(after).toContain('SUMMARY:Mexico vs Ecuador — Round of 16')
    expect(after).not.toContain('W83 vs W84')
  })
